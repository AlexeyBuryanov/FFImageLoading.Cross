...
using FFImageLoading.Ios.Extensions;
...
namespace FFImageLoading.Cross
{
    ...
    /// </summary>
    public class MvxCachedImageView : UIImageView, ICachedImageView, INotifyPropertyChanged
    {
        /// <summary>
        /// MvxCachedImageView by Daniel Luberda
        /// </summary>
        ...
        public MvxCachedImageView(ObjCRuntime.NativeHandle handle) 
            : base(handle)
        {
            Initialize();
        }
        ...
        protected MvxCachedImageView.ImageSourceBinding _lastImageSource;
        ...
        protected virtual TaskParameter GetImageLoaderForSource(MvxCachedImageView.ImageSourceBinding ffSource)
        {
        ...
        protected virtual void ConfigureImageLoader(TaskParameter imageLoader, MvxCachedImageView.ImageSourceBinding ffSource)
		{
        ...
        protected virtual MvxCachedImageView.ImageSourceBinding GetImageSourceBinding(string imagePath, Func<CancellationToken, Task<Stream>> imageStream)
        {
            if (string.IsNullOrWhiteSpace(imagePath) && imageStream == null)
                return null;

            if (imageStream != null)
                return new MvxCachedImageView.ImageSourceBinding(imageStream);
            ...
            if (imagePath.StartsWith("res:", StringComparison.OrdinalIgnoreCase))
            {
                ...
                return new MvxCachedImageView.ImageSourceBinding(ImageSource.CompiledResource, resourceName);
            }
            ...
            {
                return new MvxCachedImageView.ImageSourceBinding(ImageSource.Url, imagePath);
            }
            ...
            {
                if (uri.Scheme == "file")
                    return new MvxCachedImageView.ImageSourceBinding(ImageSource.Filepath, uri.LocalPath);

                if (uri.Scheme == "resource")
                    return new MvxCachedImageView.ImageSourceBinding(ImageSource.EmbeddedResource,imagePath);

                if (uri.Scheme == "app")
                    return new MvxCachedImageView.ImageSourceBinding(ImageSource.CompiledResource, uri.LocalPath);

                return new MvxCachedImageView.ImageSourceBinding(ImageSource.Url, imagePath);
            }

            if (!string.IsNullOrWhiteSpace(Path.GetDirectoryName(imagePath)) && File.Exists(imagePath))
                return new MvxCachedImageView.ImageSourceBinding(ImageSource.Filepath, imagePath);

            return new MvxCachedImageView.ImageSourceBinding(ImageSource.CompiledResource, imagePath);
        }
